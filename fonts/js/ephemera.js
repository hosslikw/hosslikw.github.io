...
let startEph = function () {
	if ($(window).width() >= 800) {
		...
		for (let i = 0; i < n; ++i) {
			...
			tile.animate(
				{ width: width, height: height, top: topNow },
				{ duration: 250, queue: false },
			);
			if (i % nCols == 1) {
				tile.animate(
					{ width: width, height: height, left: rand(rMin, rMax) },
					{ duration: 250, queue: false },
				);
				...
				tile.animate(
					{ width: width, height: height, left: leftNow },
					{ duration: 250, queue: false },
				);
				top = top + (height * ratio) / 3;
			}
...
let closeEph = function (dur) {
	if ($(window).width() >= 800) {
		...
		for (let i = 0; i < n; ++i) {
			let tile = $(".eph").eq(i);
			tile.animate(
				{ width: 200, height: 150, top: top, left: left },

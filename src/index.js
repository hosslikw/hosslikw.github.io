...
function showImage(imgName) {
   ...
   document.getElementById("ClickEnlargePhoto")
...
function showLargeImagePanel(imgName) {
   ...
   if (imgW > imgH && (w < h || (w > h && w / h < imgW / imgH))) {
      h2 = w * (imgH / imgW)
      $("#ClickEnlargePhoto")
         .css({ height: h2, width: w, top: (h - h2) / 2, left: 0 })
      $("#largeImg")
         .css({ height: h2 - 4, width: w - 4 })
   }
   else {
      w2 = h * (imgW / imgH)
      $("#ClickEnlargePhoto")
         .css({ width: w2, height: h, left: (w - w2) / 2, top: 0 })
      $("#largeImg")
         .css({ width: w2 - 4, height: h - 4 })
   }
}
...
let computeDocHeight = function () {
   if (document.getElementById("ClickEnlargePhoto")
      .style.visibility == "visible") {
      showLargeImagePanel($("#largeImg")
         .attr("src"))
   }
   let h = $(window)
      .height()
   let hdrHeight = $("#Home_link")
      .height()
   $("#left2")
      .css("height", h - hdrHeight - 50)
   if ($(".video")
      .length != 0) {
      let width = $("#middle")
         .width()
      $(".video")
         .css({ width: width, height: width * 0.5625 })
   }
   if ($(".video2")
      .length != 0) {
      let width = $("#middle")
...
$(".Links_ArtworkInitials ")
   .on("mouseenter", function () {
      ...
      if ($(second)
         .length > 0) {
         $(second)
            .animate({ "opacit  y": 0.0, "margin-left": "-100px" }, 800, setInvisible)
         $(second)
            .animate({ "opacity": 1.0, "margin-left": "0px" }, 10, setVisible)
      }
...
$(document)
   .rea - dy(computeDocHeight)
$(window)
   .resize(computeDocHeight)
